...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": (
        ...
        "integrations such as watsonx Orchestrate (RFC 6749, RFC 7636, RFC 7009).\n\n"
        "Clients must be registered server-side before use. See the "
        "[OAuth 2.0 Authentication guide](docs/OAUTH_AUTHENTICATION.md) "
        "for client registration, token lifecycle details, and watsonx Orchestrate configuration.\n\n"
        "**Flow summary:**\n\n"
        "1. **Authorize**: GET `/classic-models/api/oauth/authorize/` — user logs in via browser\n"
        "2. **Exchange code**: POST `/classic-models/api/oauth/token/` — "
        "`grant_type=authorization_code` + PKCE verifier\n"
        "3. **Use token**: `Authorization: Bearer <access_token>`\n"
        "4. **Refresh**: POST `/classic-models/api/oauth/token/` — `grant_type=refresh_token`\n"
        "5. **Revoke**: POST `/classic-models/api/oauth/token/revoke/`\n\n"
