...
class APITestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.order_detail = TestDataFactory.create_order_detail(order=self.order, product=self.product)
...
class ModelTestMixin:
    ...
    def assert_model_field_attributes(self, model_class, field_name, **expected_attributes):
...
class APITestMixin:
    ...
    def assert_api_response_contains(self, response, expected_keys):
        ...
        for key in expected_keys:
            assert key in response.data, f"Response missing key: {key}. Response data: {response.data}"
    ...
    def assert_api_error_response(self, response, expected_status=400):
        ...
        assert 'errors' in response.data or 'error' in response.data or 'detail' in response.data
...
class DataValidationMixin:
    ...
    def assert_decimal_precision(self, value, expected_digits, expected_places):
        ...
        max_value = Decimal('9' * (expected_digits - expected_places)) + Decimal('0.' + '9' * expected_places)
        assert value <= max_value, f"Value {value} exceeds precision {expected_digits},{expected_places}"
    ...
    def assert_max_length_constraint(self, field_value, max_length):
        ...
        if field_value is not None:
            assert len(str(field_value)) <= max_length, (
...
class DataBuilder:
    ...
    def with_product_line(self, **product_line_data):
        ...
        self.data['product_line'] = TestDataFactory.create_product_line(**product_line_data)
        return self
...
def create_test_hierarchy():
    ...
    return builder.with_user().with_office().with_employee().with_customer().with_product_line().with_product().with_order().with_order_detail().with_payment().build()
...
def assert_api_endpoint_permissions(client, url, method='GET', data=None, expected_status=200):
