...
class TestOrderAPI:
    ...
    def test_retrieve_order_authenticated(self, authenticated_api_client, order):
        ...
        url = reverse(
            "classicmodels:order-detail", kwargs={"ordernumber": order.ordernumber}
        )
        response = authenticated_api_client.get(url)
    ...
    def test_create_order_duplicate_number(
        self, authenticated_api_client, order, customer
    ):
    ...
    def test_partial_update_order_authenticated(self, authenticated_api_client, order):
        ...
        data = {"status": "Cancelled", "comments": "Order cancelled by customer"}
